...
CONFIG = {
    ...
    "max_pages": 50,              # domyślnie sprawdza dużo stron (możesz zmienić)
    "include_damaged": True,      # domyślnie wysyła uszkodzone (możesz wyłączyć)
    "ignore_age_limit": True      # domyślnie wysyła też stare ogłoszenia
}
...
def dashboard():
    ...
    return render_template_string(HTML_TEMPLATE,
                                  ...
                                  seen_ads_count=seen_ads_count,
                                  last_found_time=last_found,
...
def update_config():
    ...
    try:
        ...
        logging.info(f"🔧 Zaktualizowano konfigurację - modele: {len(CONFIG['active_models'])}")
    except Exception as e:
        message = f"❌ Błąd: {e}"
